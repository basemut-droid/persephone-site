...
| `PageHero.astro` | Slim intro banner for subpages: eyebrow + h1 + optional intro paragraph, on `--color-bg-alt` | `eyebrow?`, `title`, `intro?` | `angebote`, `beratung`, `workshops`, `kennenlernen`, `selbsthilfegruppe`, `faqs`, `disclaimer`, `impressum`, `datenschutz`, `kontakt-danke`, `blog/index`. **Not** Über uns or Kontakt — both reuse the homepage's own split-hero classes directly instead (see the "Page composition" note in `ueber-uns.astro`'s own code); **not** `/newsletter/`, which is a redirect to an external MailerLite form, not a real page (see `astro.config.mjs`) |
